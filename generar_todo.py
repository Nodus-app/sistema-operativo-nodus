...
# ── FUNCIÓN DE PROCESAMIENTO POR MES ────────────────────────────────────────
def _procesar_periodo_completo(search_dir, label):
    """Procesa todos los datos de un período y retorna el dict completo."""
    global find  # use find() with search_dir
    _find_orig = find
    def _find_local(name, sd=None):
        return _find_orig(name, sd or search_dir)
    find_fn = _find_local

    # ── VENTA ACTUAL ──────────────────────────────────────────────────────────────
    vc_path = find_fn("venta_actual.xlsx")
    if not vc_path:
        print("ERROR: no se encontro venta_actual.xlsx"); return None
    ...
    # ── CARTONES ──────────────────────────────────────────────────────────────────
    cart_records = []
    cart_path = find_fn("cartones.xlsx")
    if cart_path:
        try:
    ...
    app_path = find_fn("Registro_de_Rechazos.xlsx")
    ...
    mov_path = find_fn("movimientos.xlsx")
    if mov_path:
        try:
    ...
    com_path = find_fn("comisiones.xlsx")
    if com_path:
        try:
            ...
            ret_path = find_fn("comision_chofer.xlsx")
            if ret_path:
                try:
    ...
    return {
        'kpis': rej_kpis, 'prov': prov_metrics_list, 'motivo': by_motivo,
        'motivo_prov': motivo_prov, 'chofer': by_chofer, 'chprov': chofer_prov_map,
        'ch_det': ch_det_map, 'venta': venta_list, 'routes': route_index,
        'cli': client_map, 'cart': cart_records, 'app': app_records,
        'conc': conc_data, 'dep': dep_data, 'reinc': reinc_list, 'com': com_data,
        'periodo': PERIODO, 'chs': sorted(vc['chofer'].dropna().str.strip().unique().tolist()),
        'provs': sorted(vc['proveedor'].dropna().str.strip().unique().tolist()),
        'ch_tipos': {ch:CHOFER_TIPO.get(ch.strip(),CHOFER_TIPO.get(ch,'tercero')) for ch in vc['chofer'].dropna().str.strip().unique()},
    }
...
for (lbl, dpath) in MONTH_DIRS:
    if dpath == CURRENT_DIR:
        # Current month already processed - use existing data
        hist_data[lbl] = {
            'kpis': rej_kpis, 'prov': prov_metrics_list, 'motivo': by_motivo,
            'motivo_prov': motivo_prov, 'chofer': by_chofer, 'chprov': chofer_prov_map,
            'ch_det': ch_det_map, 'venta': venta_list, 'routes': route_index,
            ...
            'conc': conc_data, 'dep': dep_data, 'reinc': reinc_list, 'com': com_data,
            'periodo': PERIODO,
            'chs': sorted(vc['chofer'].dropna().str.strip().unique().tolist()),
            'provs': sorted(vc['proveedor'].dropna().str.strip().unique().tolist()),
            'ch_tipos': {ch:CHOFER_TIPO.get(ch.strip(),CHOFER_TIPO.get(ch,'tercero')) for ch in vc['chofer'].dropna().str.strip().unique()},
        }
        print(f"  Historial {lbl}: datos actuales reutilizados")
    else:
        try:
            d = _procesar_periodo_completo(dpath, lbl)
            if d:
                hist_data[lbl] = d
                print(f"  Historial {lbl}: OK - keys: {list(d.keys())}")
            else:
                print(f"  Historial {lbl}: funcion retorno None")
        except Exception as _eh:
            import traceback
            print(f"  Historial {lbl}: ERROR - {_eh}")
            traceback.print_exc()
